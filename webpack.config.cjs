...
glob.sync(path.resolve(__dirname, 'cases/*/index.ts').replace(/\\/g, '/'))
...
const entries = glob.sync(
    path.resolve(__dirname, 'cases/*/index.ts').replace(/\\/g, '/')
).reduce((entries, file) => {
...
// Добавляем entries для settings файлов
const settingsEntries = glob.sync(
    path.resolve(__dirname, 'cases/*/settings.ts').replace(/\\/g, '/')
).reduce((entries, file) => {
    const match = file.match(/cases[\/\\]([^\/\\]+)[\/\\]settings\.ts$/)
    if (match) {
        entries[`${match[1]}Settings`] = file
    }
    return entries
}, {})

// Объединяем все entries
const allEntries = { ...entries, ...settingsEntries }

console.log('WEBPACK ENTRIES:', allEntries);

module.exports = (env, argv) => {
    const isProduction = argv.mode === 'production'

    return Object.keys(allEntries).map(name => ({
